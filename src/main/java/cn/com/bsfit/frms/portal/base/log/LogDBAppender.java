...
public class LogDBAppender extends DBAppenderBase<ILoggingEvent> {
	...
	public void start() {
		...
		String DBType = "";
		String driverClass = "";
		DataSourceConnectionSource connectionSource = (DataSourceConnectionSource) getConnectionSource();
		DataSource dataSource = connectionSource.getDataSource();
		if(dataSource instanceof ComboPooledDataSource) {
			driverClass = ((ComboPooledDataSource) dataSource).getDriverClass();
		} else if(dataSource instanceof HikariDataSource) {
			driverClass = ((HikariDataSource) dataSource).getDriverClassName();
		}
		...
		insertExceptionSQL = LogSQLBuilder.buildInsertExceptionSQL(dbNameResolver);
		insertPropertiesSQL = LogSQLBuilder.buildInsertPropertiesSQL(dbNameResolver);
		insertSQL = LogSQLBuilder.buildInsertSQL(dbNameResolver, DBType);
		super.start();
	}
	...
	protected void bindLoggingEventWithInsertStatement(PreparedStatement stmt,
			ILoggingEvent event) throws SQLException {
		stmt.setTimestamp(TIMESTMP_INDEX, new Timestamp(event.getTimeStamp()));
		stmt.setString(FORMATTED_MESSAGE_INDEX, event.getFormattedMessage().replaceAll(LogDBUtil.PREFIX, ""));
	...
	protected void bindLoggingEventArgumentsWithPreparedStatement(PreparedStatement stmt,
	...
	protected void bindCallerDataWithPreparedStatement(PreparedStatement stmt,
	...
	protected Map<String, String> mergePropertyMaps(ILoggingEvent event) {
	 ...
	 */
	protected void updateExceptionStatement(PreparedStatement exceptionStatement,
	...
	protected short buildExceptionStatement(IThrowableProxy tp, short baseIndex, PreparedStatement insertExceptionStatement,
